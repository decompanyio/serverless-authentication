...
class Provider {
  ...
  callback(
    { code, state },
    {
      authorization_uri, profile_uri, profileMap, authorizationMethod
    },
    additionalParams
  ) {
    return new Promise((resolveAll) => {
      ...
      const createMappedProfile = (accessData) =>
        new Promise((resolve, reject) => {
          if (!accessData) {
          ...
          const url = Utils.urlBuilder(
            profile_uri,
            Object.assign({ access_token }, profile)
          )
          request.get(url, (error, httpResponse, profileData) => {
            if (error) {
              ...
              const mappedProfile = profileMap
